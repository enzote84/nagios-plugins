...
#
RESULT=$(ps aux | grep /usr/sbin/squid | grep -v grep | wc -l)
printVerbose "Variable RESULT: ${RESULT}"

# Report:
RESULTMSG="OK - El squid esta arriba | valor=${RESULT}"
RESULTCODE=${STATE_OK}
if [[ ${RESULT} -eq 0 ]]; then
  RESULTMSG="CRITICAL - El squid esta caido | valor=${RESULT}"
