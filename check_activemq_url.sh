...
#
# Example usage:
#   ./check_activemq.sh -w 300 -c 600 -H 10.0.22.1
#
# - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - - -
...
print_usage() {
  ...
  echo "  -e              Optional. List of excluded queues"
...
do
  ...
  if [[ ${QUEUECONS} -eq 0 ]]; then
    echo "1,${QUEUENAME} (${QUEUESIZE} mensajes - No tiene consumidores)" >> ${ERRORS}
    continue
  fi
  ...
  if [[ ! -s ${MESSAGESFILE} ]]; then
    echo "${STATE_UNKNOWN},${QUEUENAME} (UNKNOWN mensajes)" >> ${ERRORS}
    continue
  fi
  ...
  if [[ ${MSGAGE} -gt ${CRITTIME} ]]; then
    echo "${STATE_CRITICAL},${QUEUENAME} (${QUEUESIZE} mensajes - ${MSGAGE} segundos)" >> ${ERRORS}
  else
    if [[ ${MSGAGE} -gt ${WARNTIME} ]]; then
      echo "${STATE_WARNING},${QUEUENAME} (${QUEUESIZE} mensajes - ${MSGAGE} segundos)" >> ${ERRORS}
    fi
  fi
  # Remove temp file:
  if [[ -s ${MESSAGESFILE} ]]; then
...
if [[ -s ${ERRORS} ]]; then
  ...
  TEXTMSG=$(cat ${ERRORS} | sort -r | cut -d',' -f2 | sed -e 'H;${x;s/\n/, /g;s/^,//;p;};d')
  EXITCODE=$(cat ${ERRORS} | sort -r | cut -d',' -f1 | head -1)
  rm ${ERRORS}
fi

# Add performance metrics:
TEXTMSG="Las siguientes colas tienen problemas: ${TEXTMSG} | ${PERFORMANCE}"
